...
interface CarouselMobileScrollNodeProps {
    final: string;
}
...
export const CarouselItem = styled.div<CarouselItemProps>`
  ...
  @media ${props => props.theme.breakpoints.sm} {
    ...
    ${(props: any) => props.active === props.index ? `opacity: 1` : `opacity: 0.5`}; 
  }
`
...
export const CarouselButton = styled.button<CarouselButtonProps>`
  ...
  transform: ${(props: any) => props.active === props.index ? `scale(1.6)` : `scale(1)`};

  &:focus {
    outline: none;
  }
`
interface CarouselButtonDotProps {
    active: number;
}
